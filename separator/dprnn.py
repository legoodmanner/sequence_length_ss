...
class AdaINConv1d(nn.Module):
    def __init__(self, in_feature, nb_frames, latent_size, scalar=True ,bias=True, mode='bins'):
        super(AdaINConv1d, self).__init__()
        self.ln1 = nn.LayerNorm([in_feature, nb_frames], elementwise_affine=False)
        self.ln2 = nn.LayerNorm([in_feature, nb_frames], elementwise_affine=False)
        self.conv1 = nn.Conv1d(in_feature, in_feature, 5, stride=1, padding=2)
        self.conv2 = nn.Conv1d(in_feature, in_feature, 5, stride=1, padding=2)
        if scalar:
            self.latent_mu_fc = nn.Linear(latent_size, in_feature, bias=False)
        if bias:
            self.latent_bias_fc = nn.Linear(latent_size, in_feature, bias=False)
        self.bias = bias
        self.scalar = scalar
        #self.res_scalar = nn.Parameter(torch.rand(in_feature))
        self.drop1 = nn.Dropout(p=0.3, inplace=False)
        self.drop2 = nn.Dropout(p=0.3, inplace=False)
        
    def forward(self, _x):
        x, latent = _x
        res = x
        n_seq, nb_samples, n_channel = x.size()
        # nb_samples, n_channel, n_seq
        x = x.permute(1,2,0)
        ...
        # n_seq, nb_samples, n_channel,
        x = x.permute(2,0,1)
        # latent shape [B, 3, hidden_size]
        scaler1 = self.latent_mu_fc(latent[:,0,:]).expand(20, nb_samples, n_channel) if self.scalar else 1
        bias1 = self.latent_bias_fc(latent[:,0,:]).expand(20, nb_samples, n_channel) if self.bias else 0
        scaler2 = self.latent_mu_fc(latent[:,1,:]).expand(20, nb_samples, n_channel) if self.scalar else 1
        bias2 = self.latent_bias_fc(latent[:,1,:]).expand(20, nb_samples, n_channel) if self.bias else 0
        scaler3 = self.latent_mu_fc(latent[:,2,:]).expand(21, nb_samples, n_channel) if self.scalar else 1
        bias3 = self.latent_bias_fc(latent[:,2,:]).expand(21, nb_samples, n_channel) if self.bias else 0

        scaler = torch.cat([scaler1, scaler2, scaler3])
        #bias = torch.cat([bias1, bias2, bias3])
        x = x * scaler 
...
class DPRNN_block(nn.Module):
    def __init__(self, hidden_size, latent_size, adain_layer, nb_layers, nb_frames, unidirectional=False, ):
        ...
        self.adain_in = nn.Sequential(*([AdaINConv1d(hidden_size, nb_frames, latent_size, scalar=True, bias=False)]*adain_layer))
        ...
        #self.adain_mid = nn.Sequential(*([AdaINConv1d(nb_frames, hidden_size, latent_size, bias=True)]*adain_layer))
        ...
        #self.adain_out = nn.Sequential(*([AdaINConv1d(hidden_size, nb_frames, latent_size, bias=False)]*adain_layer))
    def finetune_freeze(self):
        for p in self.frames_lstm.parameters():
            p.requires_grad=False
        for p in self.bins_lstm.parameters():
            p.requires_grad=False
    ...
    def forward(self, x, latent):
        self.bins_lstm.flatten_parameters()
        self.frames_lstm.flatten_parameters()
        # ADAIN IN
        nb_frames, nb_samples = x.size(0), x.size(1)
        
        _x = [x, latent]
        h, _ = self.adain_in(_x)
        #######
        # LSTM FREQ
        lstm_out1, _,  = self.bins_lstm(h)
        x = torch.cat([h[...,:self.hidden_size], lstm_out1], -1)
        ...
        # LSTM FRAME
        lstm_out2, _,  = self.frames_lstm(x)
        x = torch.cat([x, lstm_out2], -1)
        ...
        return x, h, lstm_out1, lstm_out2
...
class DPRNN(nn.Module):
    ...
    def __init__(
        ...
        #self.dprnn3 = DPRNN_block(hidden_size, latent_size, adain_layer, nb_layers, nb_frames)
        #### --------------------------------------------------------
    ...
    def finetune_freeze(self):
        self.dprnn1.finetune_freeze()
        #self.dprnn2.finetune_freeze()
    ...
    def forward(self, x, latent):
        ...
        r, h, lstm_out1, lstm_out2 = self.dprnn1(x, latent)
        #x = self.dprnn2(x, latent)
        #x = self.dprnn3(x, latent)

        # OUTPUT
        x = self.fc3(r.reshape(-1, r.shape[-1]))
        x = self.bn3(x)
        ...
        return F.relu(x.permute(1, 2, 3, 0)), r.permute(1, 2, 0).unsqueeze(1), h.permute(1, 2, 0).unsqueeze(1), lstm_out1.permute(1, 2, 0).unsqueeze(1), lstm_out2.permute(1, 0, 2).unsqueeze(1)
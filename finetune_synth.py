...
separator.finetune_freeze()
separator = torch.nn.DataParallel(separator, config.parallel_cuda)
if config.separator.load_model:
    separator.load_state_dict(torch.load(config.separator.load_finetune_path))
separator.to(device)

encoder = Simple_Encoder(config.latent_size)
if config.encoder.load_model:
    encoder.load_state_dict(torch.load(config.encoder.load_finetune_path))
encoder.to(device)

"""finetuner = torch.nn.Sequential(
    torch.nn.Conv2d(2,128,3,1,1),
    torch.nn.ReLU(),
    torch.nn.Conv2d(128,128,3,1,1),
    torch.nn.ReLU(),
    torch.nn.Conv2d(128,2,3,1,1)
)
finetuner = torch.nn.DataParallel(finetuner, config.parallel_cuda)
finetuner.to(device)"""

"""train_set = MUSDB18_seg(subset='train', mode='pair')
test_set = MUSDB18_seg(subset='test', mode='pair')"""
dataset = ChoralSingingDataset(mode= config.loader_mode) #root='/home/lego/NAS189/home/JSBChorale/npy_Aiko/')
split = (len(dataset) // 3) * 2
...
for epoch in range(1, config.n_epoch):
    ...
    writter.add_scalar('train_sdr_a', sdr_a.median(), epoch)
    if epoch % 10 == 0:
    ...
    torch.save(separator.state_dict(), config.separator.save_finetune_path)
    torch.save(encoder.state_dict(), config.encoder.save_finetune_path)
    #torch.save(finetuner.state_dict(), 'params/finetuner.pkl')

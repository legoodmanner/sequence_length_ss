...
class PITLoss(nn.Module):
    def __init__(self, criterion, perm_num=2, reduction='mean'):
        super(PITLoss, self).__init__()
        self.perm_num = perm_num
        self.criterion = criterion
        self.reduction = reduction
        
    def forward(self, x, y, pred_a, pred_b):
        b = y.size(0)
        batch_loss = torch.zeros(1).cuda(2)
        perm = []
        for i in range(b):
            l1 = self.criterion(pred_a[i], x[i]) #+ self.criterion(pred_b[i], y[i])
            l2 = self.criterion(pred_a[i], y[i]) #+ self.criterion(pred_b[i], x[i])

...
def move_data_to_device(x, device):
    if 'float64' or 'long' in str(x.dtype):
        x = x.float()
...
class STFT(nn.Module):
    def __init__(self, cuda, **kwargs):
        ...
        self.shift = Shift(63488) #7-47100 20-63488 40-60416 
        self.flip_channel = FlipChannels()
        self.flip_sign = FlipSign()
        self.remix = Remix()
        self.scale = Scale()

    def randomApply(self, func, x, p):
        if torch.rand(1).item() < p:
            return func(x)
        else:
            return x

    def forward(self, y, instrument_indice, augmentation=True):
        """
        Args: [B, n_channel, time]
            [B, source, n_channel, time]
        Returns:
        ...
        if augmentation:
            y = self.randomApply(self.shift, y, p=1)
            y = self.randomApply(self.flip_channel, y, p=0.5)
            # y = self.randomApply(self.flip_sign, y, p=0.2)
            y = self.randomApply(self.remix, y, p=1)
            # y = self.randomApply(self.scale, y, p=0.5)

        shape = y.size()
        nb_samples, source, nb_channels, nb_timesteps = shape
        
        

        # ind = source_indice.view(nb_samples,1,1,1).expand(-1,-1, nb_channels, nb_timesteps)
        # target = y.gather(1, ind).squeeze(1) # nb_samples, 1, nb_channels, nb_timesteps -> nb_samples, nb_channels, nb_timesteps
        mix = y.sum(1) # [B, n_channel, time]
        target = y[:,instrument_indice]
        ...
        for x in [target, mix]:
            ...
            complex_stft = torch.stft(
                    ...
                    **self.kwargs,
                    return_complex = True,
                    window = torch.hann_window(self.n_fft, requires_grad=False, device=x.device),
                    
                )
            
            stft_f = torch.view_as_real(complex_stft)
            # unpack batch
            stft_f = stft_f.reshape((nb_samples, nb_channels) + stft_f.shape[-3:])
            output.append(stft_f)
        return output[0], output[1] # target, mix
        ...
        complex_stft = torch.stft(
                ...
                **self.kwargs,
                return_complex = True,
                window = torch.hann_window(self.n_fft, requires_grad=False, device=y.device),
                
            )
        stft_f = torch.view_as_real(complex_stft)
        # unpack batch
        stft_f = stft_f.reshape((nb_samples, source, nb_channels) + stft_f.shape[-3:])
        return stft_f, stft_f.sum(1) # target, mix  """
...
def augment_gain(x, low: float = 0.25, high: float = 1.25):
    """Applies a random gain between `low` and `high`"""
    g = low + torch.rand(1) * (high - low)
    return x * g


def augment_channelswap(x):
    """Swap channels of stereo signals with a probability of p=0.5"""
    if x.shape[0] == 2 and torch.tensor(1.0).uniform_() < 0.5:
